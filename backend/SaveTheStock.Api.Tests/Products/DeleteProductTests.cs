...
public sealed class DeleteProductTests : IClassFixture<SaveTheStockApiFactory>
{
    private readonly SaveTheStockApiFactory _factory;

    public DeleteProductTests(SaveTheStockApiFactory factory) => _factory = factory;

    [Fact]
    public async Task WhenUnauthorized_ShouldReturn401()
    {
        var client = _factory.CreateClient();
        var res = await client.DeleteAsync($"/api/products/{Guid.NewGuid()}");
        Assert.Equal(HttpStatusCode.Unauthorized, res.StatusCode);
    }

    [Fact]
    public async Task WhenExists_ShouldReturn204()
    {
        ...
        var res = await client.DeleteAsync($"/api/products/{productId}");
        Assert.Equal(HttpStatusCode.NoContent, res.StatusCode);
    }

    [Fact]
    public async Task WhenMissing_ShouldStillReturn204()
    {
        var client = (await CreateAuthedClientAsync()).Client;
